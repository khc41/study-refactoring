...
## 9. 죽은 코드 제거하기
- 사용되지 않는 코드는 성능 상의 영향은 없지만 소프트웨어의 동작을 이해하는 데 커다란 걸림돌이 될 수 있다.
...
## 느낀점
- 중첩 함수를 사용하는 javascript와는 문법 자체가 달라 신경 쓸 포인트가 줄었다는 것이 좋았다.
- 반복문 쪼개기가 처음 읽었을 땐 왜 필요한지 의문이 들었지만, 문장 슬라이스하기와 함수로 추출하기를 거쳐서 코드가 많이 깔끔해진 것을 체감할 수 있었고, 실무에서도 적용해보고 싶다는 생각이 들었다.
