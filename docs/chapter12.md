## 1. 메서드 올리기
- 중복되었다는 것은 한쪽의 변경이 다른쪽에는 반영되지 않을 수 있다는 위험을 수반하는데, 일반적으로 중복을 찾기가 쉽지 않다.
...
## 2. 필드 올리기
- 필드들이 비슷한 방식으로 쓰인다고 판단되면 필드를 서브클래스에서 슈퍼클래스로 끌어올리자.
...
## 6. 타입 코드를 서브클래스로 바꾸기
- 타입 코드에 따라 동작이 달라져야 하는 함수가 여러 개이거나 특정 타입에서만 의미가 있는 값을 사용하는 필드나 메서드가 있을 때 서브 클래스로 생성하면 유용하다.
- 대상 클래스에 직접 적용하거나 타입 코드 자체를 클래스로 정의해 속성으로 부여하는 방법이 있다.
...
## 10. 서브클래스를 위임으로 바꾸기
- 상속은 달라져야 하는 이유가 여러 개여도 단 하나의 이유만 선택해 기준으로 삼을 수 밖에 없다.
- 상속은 클래스들의 관계를 아주 긴밀하게 결합해 부모를 수정하면 자식들의 기능을 해치기가 쉽다.
- 위임은 두 문제를 모두 해결해준다.
### 절차
1. 생성자를 호출하는 곳이 많다면 생성자를 팩터리 함수로 바꾼다.
2. 위임으로 활용할 빈 클래슬르 만든다. 이 클래스의 생성자는 서브클래스에 특화된 데이터를 전부 받아야 하며, 보통은 슈퍼클래스를 가리키는 역참조도 필요하다.
...
- 제대로된 상속은 슈퍼클래스가 사용되는 모든 곳에서 서브클래스의 인스턴스를 대신 사용해도 이상없이 동작해야 한다.
...
## 느낀점
- 처음에 서브클래스 책임 오류라고 서브 클래스가 메서드를 구현하지 않으면 오류를 발생시키는 부분이 있는데 이를 보면서 이러한 코드 기법들이 단순히 본인이 코드를 잘 이해하기 위한 수단을 넘어서 협업 시 다른 개발자와의 명확한 의사소통을 위해서도 중요하다는 것을 느꼈다.